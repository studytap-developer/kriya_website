...
const featuredProducts = [
  { name: 'Gold Bangles', price: 'Rs. 271,452', image: one },
  { name: 'Long Black Beads', price: 'Rs.117,392', image: two },
];
...
const FeaturedProducts = () => {
  ...
  const handleTryThisClick = (image) => {
    setSelectedNecklace(image);
    setIsModalOpen(true);
  };
  ...
  return (
    <section className="py-16 bg-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
          {featuredProducts.map((product, index) => (
            <div key={index} className="bg-gray-100 rounded-lg p-4 text-center shadow-lg">
              ...
              <p className="text-gray-500">{product.price}</p>
              <button
                onClick={() => handleTryThisClick(product.image)}
      ...
      <TryOnModal
        ...
        necklaceImage={selectedNecklace}
      />
    </section>
  );
};
